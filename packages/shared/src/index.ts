...
export type UserRole =
  'super_admin' | 'manager' | 'seller' | 'warehouse' | 'accountant' | 'wholesale';
...
/** Inventory fields embedded in product.create — applied in the same database
 * transaction as the catalog row, so a product never exists without its
...
export interface ProductUpdatePayload {
  ...
  categoryId?: string;
  status?: 'active' | 'hidden';
  inventory?: InventoryUpdateMetadataPayload;
}
...
/** Decisions a client may send to POST /sync/conflicts/:id/resolve. */
export const SYNC_CONFLICT_DECISIONS = [
  'retry',
  'reject',
  'accept_server_state',
  'create_new_draft',
] as const;
export type SyncConflictDecision = (typeof SYNC_CONFLICT_DECISIONS)[number];
/** SyncChange actions delivered by GET /sync/pull. `deleted` means the client
 * must drop the entity from its cache; the other two carry a full snapshot. */
export type SyncChangeAction = 'created' | 'updated' | 'deleted';
...
export function validateSyncOperationEnvelope(
  input: unknown,
): { ok: true; value: SyncOperationEnvelope } | { ok: false; error: string } {
  ...
  if (typeof value.operationId !== 'string' || !operationIdPattern.test(value.operationId))
    return { ok: false, error: 'operationId must be 8-100 safe characters' };
  if (typeof value.deviceId !== 'string' || !deviceIdPattern.test(value.deviceId))
    return { ok: false, error: 'deviceId is invalid' };
  if (
    typeof value.type !== 'string' ||
    !(SYNC_OPERATION_TYPES as readonly string[]).includes(value.type)
  )
    return { ok: false, error: 'operation type is unsupported' };
