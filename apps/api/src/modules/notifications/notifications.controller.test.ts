...
describe('NotificationsController', () => {
  function setup() {
    const notifications = {
      enqueueTest: vi.fn(async (channel: string, message: string, mobile?: string) => ({
        id: 'job-1',
        channel,
        message,
        mobile,
  ...
  it('queues a guarded provider test message', async () => {
    ...
    const body: TestNotificationDto = {
      channel: NotificationTestChannel.telegram,
      message: 'پیام تست',
      mobile: undefined,
    };
    ...
    expect(result).toEqual({
      ok: true,
      data: { jobId: 'job-1', channel: 'telegram', queued: true },
    });
  ...
  it('returns the retry result for an existing or missing failed job', async () => {
    const { controller, notifications } = setup();
    await expect(controller.retry('failed-1')).resolves.toEqual({
      ok: true,
      data: { retried: true },
    });
    await expect(controller.retry('missing')).resolves.toEqual({
      ok: false,
      data: { retried: false },
    });
  ...
  it('passes failed-job limit and exposes queue health', async () => {
    const { controller, notifications } = setup();
    await expect(controller.failed('25')).resolves.toEqual({
      ok: true,
      data: [{ id: 'failed-1', attemptsMade: 25 }],
    });
    await expect(controller.health()).resolves.toEqual({
      ok: true,
      data: { channels: {}, queue: { waiting: 2 } },
    });
  ...
  it('returns queue counts', async () => {
    ...
    await expect(controller.queue()).resolves.toEqual({
      ok: true,
      data: { waiting: 2, active: 1, completed: 4, failed: 0, delayed: 0 },
    });
  });
});
