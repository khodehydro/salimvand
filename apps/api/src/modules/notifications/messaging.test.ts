...
describe('SMS templates', () => {
  it('renders known placeholders and leaves unknown ones untouched', () => {
    expect(renderSmsTemplate('فاکتور {invoice_number} به مبلغ {amount} ریال: {link}', { invoice_number: 'INV-0001', amount: 1200, link: 'https://x/i/abc' }))
...
describe('Telegram bot commands', () => {
  it('parses commands, mentions and arguments', () => {
    ...
    expect(parseTelegramCommand('/invoice abc123')).toEqual({ command: 'invoice', argument: 'abc123' });
  ...
  it('answers stock, low stock and daily sales from the database', async () => {
    const prisma = {
      ...
      invoice: { count: vi.fn(async () => 3), aggregate: vi.fn(async () => ({ _sum: { total: 9_000_000n } })) },
    };
    ...
    expect(await service.answerCommand('/sales')).toContain('9000000000'.replace('9000000000', String(9_000_000)));
  ...
  it('builds the invoice link and shows help for anything else', async () => {
    const service = serviceWith({ inventoryItem: { count: async () => 0 }, invoice: { count: async () => 0, aggregate: async () => ({ _sum: { total: null } }) } });
...
describe('messaging logs', () => {
  it('masks the mobile number in the SMS log and stringifies ids', async () => {
    const prisma = { smsLog: { findMany: async () => [ { id: 7n, mobile: '09123456789', template: 'invoice.issued', status: 'sent', provider: 'kavenegar', error: null, createdAt: new Date() } ] } };
    const rows = await serviceWith(prisma).smsLogs();
    expect(rows).toEqual([ { id: '7', mobile: '091***89', template: 'invoice.issued', status: 'sent', provider: 'kavenegar', error: null, createdAt: expect.any(Date) } ]);
  });

  it('returns a short preview for channel logs', async () => {
    const prisma = { telegramLog: { findMany: async () => [ { id: 3n, channel: 'bale', status: 'failed', message: 'x'.repeat(200), error: 'timeout', createdAt: new Date() } ] } };
