...
describe('DashboardService', () => {
  it('aggregates product, inventory, threshold and recent transaction metrics', async () => {
    ...
    prisma.product.count.mockResolvedValue(12); prisma.inventoryItem.count.mockResolvedValue(20); prisma.inventoryItem.findMany.mockResolvedValueOnce([{ id: 'i1', quantity: 2, minStock: 3, product: { name: 'لنت', code: 'BRK-1' }, brand: { name: 'اصلی' }, location: { code: 'A-1', name: 'قفسه ۱' } }, { id: 'i2', quantity: 10, minStock: 3, product: { name: 'فیلتر', code: 'FLT-1' }, brand: { name: 'بوش' }, location: null }]).mockResolvedValueOnce([{ quantity: 12, brand: { name: 'اصلی' } }, { quantity: 8, brand: { name: 'بوش' } }]); prisma.inventoryTransaction.findMany.mockResolvedValue([{ id: '1', quantityChange: 2 }]);
    await expect(service.summary()).resolves.toEqual({ ok: true, data: { products: 12, inventoryItems: 20, lowStock: 1, lowStockItems: [{ id: 'i1', quantity: 2, minStock: 3, product: { name: 'لنت', code: 'BRK-1' }, brand: { name: 'اصلی' }, location: { code: 'A-1', name: 'قفسه ۱' } }], stockComposition: [{ quantity: 12, brand: { name: 'اصلی' } }, { quantity: 8, brand: { name: 'بوش' } }], recentTransactions: [{ id: '1', quantityChange: 2 }] } });
  ...
  it('groups the sales trend by day and returns JSON-safe amounts', async () => {
    ...
    expect(prisma.invoice.findMany.mock.calls[0][0].where.issuedAt.lte.toISOString()).toBe('2026-08-02T23:59:59.999Z');
  });
  ...
  it('groups inventory movements by day and direction', async () => {
    ...
    expect(prisma.inventoryTransaction.findMany.mock.calls[0][0].where.createdAt.lte.toISOString()).toBe('2026-08-01T23:59:59.999Z');
  });
  ...
  it('calculates daily revenue, cost and gross profit', async () => {
    ...
    expect(prisma.invoice.findMany.mock.calls[0][0].where.issuedAt.lte.toISOString()).toBe('2026-08-01T23:59:59.999Z');
  });
});
