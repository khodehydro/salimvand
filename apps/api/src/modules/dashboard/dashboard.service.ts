...
export class DashboardService {
  ...
  async summary() {
    const [products, inventoryItems, lowStockItems, stockComposition, recentTransactions] = await Promise.all([
      ...
      this.prisma.inventoryItem.findMany({ where: { isActive: true, quantity: { gt: 0 } }, select: { quantity: true, brand: { select: { name: true } } } }),
    ...
    return { ok: true, data: { products, inventoryItems, lowStock: lowStock.length, lowStockItems: lowStock, stockComposition, recentTransactions } };
  }
}
