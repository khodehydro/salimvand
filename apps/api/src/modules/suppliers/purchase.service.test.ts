...
describe('PurchaseService validation', () => {
  ...
  it('rejects supplier invoice payment above the outstanding balance', async () => {
    ...
    const tx = { purchaseInvoice: { findUnique: vi.fn(async () => ({ id: 'purchase-1', status: 'issued', supplierId: 'supplier-1', total: 1000n, paidAmount: 900n })), updateMany }, supplierPayment: { create: vi.fn() } };
    const prisma = { $transaction: vi.fn(async (callback: (client: typeof tx) => Promise<unknown>) => callback(tx)) };
    await expect(new PurchaseService(prisma as never).pay('purchase-1', 101, 'cash', undefined, 'actor-id')).rejects.toThrow('مبلغ پرداخت بیشتر از بدهی فاکتور است');
  ...
  it('updates the invoice and records a supplier payment atomically', async () => {
    ...
    const invoice = { id: 'purchase-1', status: 'issued', supplierId: 'supplier-1', total: 1000n, paidAmount: 400n };
    const updated = { ...invoice, paidAmount: 700n };
    const findUnique = vi.fn().mockResolvedValueOnce(invoice).mockResolvedValueOnce(updated);
    const tx = { purchaseInvoice: { findUnique, updateMany }, supplierPayment: { create: supplierPayment } };
    const prisma = { $transaction: vi.fn(async (callback: (client: typeof tx) => Promise<unknown>) => callback(tx)) };
    const result = await new PurchaseService(prisma as never).pay('purchase-1', 300, 'transfer', 'تسویه', 'actor-id');
    expect(result.data.invoice.paidAmount).toBe(700n);
    expect(updateMany).toHaveBeenCalledTimes(1);
    expect(supplierPayment).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ amount: 300n, method: 'transfer' }) }));
  });
});
