...
describe('UsersController', () => {
  it('routes user list and activity', async () => {
    ...
    const activity = vi.fn(async () => ({ ok: true, data: { user: {}, auditLogs: [], transactions: [], invoices: [] } }));
  ...
  it('routes user creation and update', async () => {
    ...
    await controller.create({ name: 'کاربر', username: 'user1', password: 'password123', role: 'seller' }, request);
    await controller.update('u1', { name: 'کاربر ویرایش‌شده' }, request);
    expect(create).toHaveBeenCalledWith({ name: 'کاربر', username: 'user1', password: 'password123', role: 'seller' }, 'admin-1', '127.0.0.1');
