...
describe('public invoice token', () => {
  ...
  it('matches the original token without accepting a different token', () => {
    const created = createPublicToken();
    expect(matchesPublicToken(created.token, created.hash)).toBe(true);
    expect(matchesPublicToken(`${created.token}x`, created.hash)).toBe(false);
  });
  it('produces deterministic hashes for database lookup', () => {
    expect(hashPublicToken('sample')).toBe(hashPublicToken('sample'));
    expect(hashPublicToken('sample')).not.toBe(hashPublicToken('other'));
  });
});
