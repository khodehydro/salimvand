...
export function calculateInvoiceTotals(lines: readonly InvoiceLineInput[], discount: bigint = 0n): InvoiceTotals {
  if (discount < 0n) throw new Error('تخفیف نمی‌تواند منفی باشد');
  const subtotal = lines.reduce((sum, line) => {
    if (!Number.isInteger(line.quantity) || line.quantity <= 0) throw new Error('تعداد کالا باید عدد صحیح مثبت باشد');
