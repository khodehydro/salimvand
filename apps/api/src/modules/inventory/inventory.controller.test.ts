...
describe('InventoryController', () => {
  it('allows accountants to read purchase inventory without granting write access', () => {
    expect(Reflect.getMetadata(ROLES_KEY, InventoryController.prototype.list)).toEqual([
      'warehouse',
      'accountant',
    ]);
  ...
  it('passes inventory filters to the service', async () => {
    ...
    await expect(controller.list('لنت', 'brand-1', 'location-1', 'low')).resolves.toEqual({
      ok: true,
      data: [{ q: 'لنت', brandId: 'brand-1', locationId: 'location-1', status: 'low' }],
    });
  });
