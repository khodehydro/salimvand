...
export class CatalogService {
  ...
  async sitemap() {
    const [products, categories] = await Promise.all([this.prisma.product.findMany({ where: { status: 'active', deletedAt: null }, select: { slug: true, updatedAt: true } }), this.prisma.category.findMany({ where: { isActive: true }, select: { slug: true, updatedAt: true } })]);
  ...
  async listFilters() {
    const [categories, vehicles, brands] = await Promise.all([
      ...
      this.prisma.vehicleMake.findMany({ orderBy: { name: 'asc' }, include: { models: { orderBy: { name: 'asc' }, include: { trims: { orderBy: { name: 'asc' } } } } } }),
