...
export class SearchService {
  ...
  async all(query: string) {
    const q = query.trim();
    if (q.length > 100) throw new BadRequestException('عبارت جست‌وجو بیش از حد مجاز است');
    if (!q)
      return {
        ok: true,
    ...
    const [products, customers, invoices, brands, locations] = await Promise.all([
      this.prisma.product.findMany({
        ...
        select: {
          ...
          code: true,
          slug: true,
          inventoryItems: { select: { quantity: true, brand: { select: { name: true } } } },
        },
      }),
      ...
      this.prisma.invoice.findMany({
        ...
        select: { id: true, number: true, customerName: true, total: true, paymentStatus: true },
      }),
