...
export function corsOrigins(value: string | undefined, nodeEnv = process.env.NODE_ENV): true | string[] {
...
export function validateRuntimeConfig(env: NodeJS.ProcessEnv = process.env): void {
  ...
  if (missing.length) throw new BadRequestException(`تنظیمات ضروری محیط کامل نیست: ${missing.join(', ')}`);
}
