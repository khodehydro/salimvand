...
async function main() {
  const category = await prisma.category.upsert({
    where: { code: 'INT' },
    update: {},
    create: { name: 'لوازم داخلی خودرو', slug: 'lavazem-dakheli-khodro', code: 'INT' },
  });
  await prisma.brand.upsert({
    where: { name: 'سلیم وند' },
    update: {},
    create: { name: 'سلیم وند' },
  });
  await prisma.vehicleMake.upsert({
    where: { name: 'ایران خودرو' },
    update: {},
    create: {
      name: 'ایران خودرو',
      models: {
        create: [{ name: 'پژو ۲۰۶' }, { name: 'پژو ۴۰۵' }, { name: 'سمند' }, { name: 'دنا' }],
      },
    },
  });
