...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = { '@context': 'https://schema.org', '@type': 'Product', name: product.name, sku: product.code, description: product.description, image: product.images?.map((image) => image.path), brand: product.brands?.length ? { '@type': 'Brand', name: product.brands[0].name } : undefined }; const breadcrumb = { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'خانه', item: 'https://salimvand.ir' }, { '@type': 'ListItem', position: 2, name: 'کاتالوگ محصولات', item: 'https://salimvand.ir/#catalog' }, { '@type': 'ListItem', position: 3, name: product.name, item: `https://salimvand.ir/product/${product.slug}` }] }; return <main className="shell"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} /><header><strong>فروشگاه سلیم وند</strong><span>آذین خودرو · میاندوآب</span></header><article className="product-page"><nav className="breadcrumb"><a href="/">خانه</a><span>←</span><a href="/#catalog">کاتالوگ</a><span>←</span><span>{product.name}</span></nav><div className="product-gallery">{product.images?.length ? product.images.map((image) => <img key={image.path} src={image.path} alt={image.alt ?? product.name} />) : <div className="image-placeholder">تصویر محصول</div>}</div><p className="eyebrow">کاتالوگ قطعات خودرو</p><h1>{product.name}</h1><p className="code">کد محصول: {product.code}</p><p>{product.description ?? `برای استعلام ${product.name} با فروشگاه آذین خودرو سلیم وند تماس بگیرید.`}</p><div className="status">وضعیت: {product.availability === 'in_stock' ? 'موجود' : 'استعلام موجودی'}</div><h2>برندها</h2><ul>{product.brands.map((brand) => <li key={brand.name}>{brand.name} — {brand.inStock ? 'موجود' : 'ناموجود'}</li>)}</ul><a className="contact" href="tel:">تماس برای استعلام قیمت</a></article></main>;
}
