...
type Product = { slug: string; name: string; code: string; availability: string; images: Array<{ path: string; alt?: string }>; category: { name: string } };
...
export default async function VehiclePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="shell"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} /><header><strong>فروشگاه سلیم وند</strong><span>کاتالوگ خودرو</span></header><section className="catalog"><nav className="breadcrumb"><a href="/">خانه</a><span>←</span><span>{data.vehicle.name}</span></nav><p className="eyebrow">قطعات خودرو</p><h1>قطعات مناسب {data.vehicle.name}</h1>{data.products.length ? <div className="product-grid">{data.products.map((product) => <a className="product-card" href={`/product/${product.slug}`} key={product.slug}>{product.images[0] ? <img src={product.images[0].path} alt={product.images[0].alt ?? product.name} /> : <div className="image-placeholder">قطعه خودرو</div>}<small>{product.category.name}</small><h2>{product.name}</h2><code>{product.code}</code><b className="available">{product.availability === 'in_stock' ? 'موجود' : 'استعلام موجودی'}</b></a>)}</div> : <div className="placeholder">هنوز قطعه‌ای برای این خودرو ثبت نشده است.</div>}</section></main>;
}
