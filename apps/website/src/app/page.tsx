...
type Product = { slug: string; name: string; code: string; availability: string; brands: Array<{ name: string; inStock: boolean }>; compatibilities?: Array<{ model: { name: string; make: { name: string } }; trim?: { name: string } | null }>; images: Array<{ path: string; thumbnailPath?: string; alt?: string }>; category: { name: string } };
...
export default async function HomePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <main className="site-shell">
    ...
    <section className="catalog-section" id="catalog"><div className="section-heading"><div><span className="eyebrow">کاتالوگ قطعات</span><h2>قطعهٔ موردنظرت را پیدا کن</h2><p>{formatPersianNumber(products.total)} نتیجه · فیلترها در آدرس صفحه ذخیره می‌شوند</p></div><span className="price-note">قیمت فقط با استعلام</span></div>
      ...
      {products.items.length ? <div className="product-grid">{products.items.map((product) => <a className="product-card" href={`/product/${product.slug}`} key={product.slug}><div className="product-image">{product.images[0] ? <img src={product.images[0].thumbnailPath ?? product.images[0].path} srcSet={product.images[0].thumbnailPath ? `${product.images[0].thumbnailPath} 400w, ${product.images[0].path} 900w` : undefined} sizes="(max-width: 620px) 50vw, (max-width: 900px) 33vw, 25vw" alt={product.images[0].alt ?? product.name} loading="lazy" /> : <span>قطعه خودرو</span>}<span className={`status-badge ${product.availability}`}>{product.availability === 'in_stock' ? 'موجود' : product.availability === 'low_stock' ? 'موجود (کم)' : product.availability === 'coming_soon' ? 'به‌زودی' : 'ناموجود'}</span></div><span className="category-label">{product.category.name}</span><h3>{product.name}</h3><p className="compatibility">{product.compatibilities?.slice(0, 2).map((item) => `${item.model.make.name} ${item.model.name}`).join(' · ') || 'مناسب خودروهای داخلی'}</p><div className="brand-list">{product.brands.slice(0, 4).map((brand) => <span className={brand.inStock ? 'brand-in' : 'brand-out'} key={brand.name}>{brand.inStock ? '✓' : '×'} {brand.name}</span>)}</div><div className="card-footer"><code>{product.code}</code><span>استعلام قیمت <b>←</b></span></div></a>)}</div> : <div className="empty-state"><b>قطعه‌ای با این مشخصات پیدا نشد.</b><span>فیلترها را پاک کنید یا عبارت دیگری جست‌وجو کنید.</span><a href="#catalog">پاک کردن فیلترها</a></div>}
