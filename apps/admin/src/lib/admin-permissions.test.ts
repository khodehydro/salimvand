...
describe('admin page permissions', () => {
  ...
  it('shows each role only its permitted dashboard datasets', () => {
    expect(dashboardCapabilities('seller')).toMatchObject({
      canViewSales: true,
    ...
    expect(dashboardCapabilities('warehouse')).toMatchObject({
      canViewSales: false,
      canViewInventory: true,
      canViewProfit: false,
      canViewDebtors: false,
    });
    expect(dashboardCapabilities('accountant')).toMatchObject({
      canViewSales: true,
      canViewInventory: false,
      canViewProfit: true,
      canViewDebtors: true,
    });
    expect(dashboardCapabilities('manager')).toMatchObject({
      canViewSales: true,
      canViewInventory: true,
      canViewProfit: true,
      canViewHealth: true,
  ...
  it('separates invoice read, issue, payment, and void capabilities', () => {
    expect(invoiceCapabilities('accountant')).toEqual({
      canCreate: false,
      canPay: true,
      canResend: true,
      canVoid: false,
    });
    expect(invoiceCapabilities('seller')).toEqual({
      canCreate: true,
      canPay: true,
      canResend: true,
      canVoid: false,
