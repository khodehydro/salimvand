...
import { discountedUnitPrice, invoiceTotals, isValidIranMobile, lineTotal, paymentTotal, remainingDebt } from './invoice-math';
...
describe('invoice math', () => {
  ...
  it('sums lines and clamps the invoice discount to the subtotal', () => {
    const lines = [ { salePrice: 1_200_000, quantity: 2, lineDiscount: 100_000 }, { salePrice: 800_000, quantity: 1, lineDiscount: 0 } ];
    expect(invoiceTotals(lines, 200_000)).toEqual({ subtotal: 3_100_000, discount: 200_000, total: 2_900_000 });
  ...
  it('ignores negative or unparsable payment amounts', () => {
    expect(paymentTotal([ { method: 'cash', amount: '1500000' }, { method: 'card', amount: '-40' }, { method: 'cash', amount: 'abc' } ])).toBe(1_500_000);
  });
  ...
  it('reports the debt left after mixed payments', () => {
    expect(remainingDebt(3_000_000, [ { method: 'cash', amount: '1000000' }, { method: 'card', amount: '1500000' } ])).toBe(500_000);
