...
describe('admin API client', () => {
  ...
  it('keeps an explicitly configured development API and strips trailing slashes', () => {
    expect(resolveApiUrl(' http://localhost:4000/api/v1/ ', false)).toBe('http://localhost:4000/api/v1');
  ...
  it('turns browser network failures into an actionable Persian message', async () => {
    ...
    await expect(api('/auth/login', { method: 'POST', body: '{}' })).rejects.toThrow('ارتباط با سرور برقرار نشد');
  });

  it('preserves API validation messages', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: { message: 'نام کاربری یا رمز عبور نادرست است' } }), {
      status: 401,
      headers: { 'content-type': 'application/json' },
    })));
    await expect(api('/auth/login', { method: 'POST', body: '{}' })).rejects.toThrow('نام کاربری یا رمز عبور نادرست است');
  });
});
