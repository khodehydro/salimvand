...
export function DashboardPage({
  ...
  return (
    <section className="dash-page">
      ...
      {backupOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal-card"><h3>پشتیبان‌گیری کامل</h3><p>{backupError || (backupDone ? 'پشتیبان آمادهٔ دریافت است.' : 'در حال آماده‌سازی دیتابیس و فایل‌های رسانه‌ای...')}</p><div className="progress"><span style={{ width: `${backupProgress}%` }} /></div><strong>{faNum(backupProgress)}٪</strong>{backupDone && <><button className="btn primary" onClick={() => void downloadFile('/settings/backup/download', 'salimvand-backup.tar.gz')}>دانلود فایل پشتیبان</button><button className="btn" onClick={async () => { try { const result = await api<{ data: { webViewLink?: string } }>('/settings/backup/google-drive', { method: 'PUT' }); if (result.data.webViewLink) window.open(result.data.webViewLink, '_blank', 'noopener,noreferrer'); } catch (error) { setBackupError((error as Error).message); } }}>ارسال به Google Drive</button></>} {(backupDone || backupError) && <button className="btn" onClick={() => setBackupOpen(false)}>بستن</button>}</div></div>}
