...
type Store = { phone: string; address: string; website: string };
...
export function WholesalePage() {
  const [rows, setRows] = useState<Row[]>([]); const [store, setStore] = useState<Store>({ phone: '', address: '', website: 'https://salimvand.ir' }); const [q, setQ] = useState(''); const [category, setCategory] = useState(''); const [vehicle, setVehicle] = useState('');
  useEffect(() => { void api<{ data: Row[]; store: Store }>('/products/wholesale').then((r) => { setRows(r.data); setStore(r.store); }); }, []);
