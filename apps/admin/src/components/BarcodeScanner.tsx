...
export function BarcodeScanner({ onCode }: { onCode: (code: string) => void }) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setActive(false);
      setError('دسترسی به دوربین ممکن نیست یا اسکنر پشتیبانی نمی‌شود');
    }
  };
  const stop = async () => {
    if (scanner.current) {
  ...
  return (
    <div className="scanner">
      ...
      {active ? (
        <button className="outline" onClick={() => void stop()}>
          توقف اسکن
        </button>
      ) : (
        <button onClick={() => void start()}>اسکن با دوربین</button>
      )}
