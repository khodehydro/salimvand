...
type Page = 'dashboard' | 'products' | 'inventory' | 'invoices' | 'media' | 'references' | 'reports' | 'settings';
type NavItem = { id: Page; label: string; icon: string; roles?: UserRole[] };
const navItems: NavItem[] = [
  { id: 'dashboard', label: 'داشبورد', icon: '⌂' }, { id: 'invoices', label: 'فروش و فاکتورها', icon: '▤' }, { id: 'products', label: 'کاتالوگ محصولات', icon: '▦' }, { id: 'inventory', label: 'انبار و موجودی', icon: '⌗' }, { id: 'media', label: 'رسانه‌ها', icon: '◫' }, { id: 'references', label: 'برندها و خودروها', icon: '◇', roles: ['manager', 'super_admin'] }, { id: 'reports', label: 'گزارش‌ها', icon: '◒', roles: ['manager', 'super_admin', 'accountant'] }, { id: 'settings', label: 'تنظیمات', icon: '⚙', roles: ['manager', 'super_admin'] },
];
const pageTitles: Record<Page, string> = { dashboard: 'داشبورد', products: 'کاتالوگ محصولات', inventory: 'انبار و موجودی', invoices: 'فروش و فاکتورها', media: 'رسانه‌ها', references: 'برندها و خودروها', reports: 'گزارش‌ها', settings: 'تنظیمات سیستم' };

function CommandPalette({ items, onSelect, onClose }: { items: NavItem[]; onSelect: (page: Page) => void; onClose: () => void }) {
  const [query, setQuery] = useState(''); const filtered = items.filter((item) => item.label.includes(query) || item.id.includes(query));
  return <div className="palette-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="command-palette" role="dialog" aria-label="جست‌وجوی پنل"><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجو در پنل..." onKeyDown={(event) => { if (event.key === 'Escape') onClose(); if (event.key === 'Enter' && filtered[0]) { onSelect(filtered[0].id); onClose(); } }} /> <div className="palette-results">{filtered.map((item, index) => <button key={item.id} onClick={() => { onSelect(item.id); onClose(); }}><span className="palette-key">{index + 1}</span><span>{item.icon}</span>{item.label}<kbd>{index < 5 ? index + 1 : ''}</kbd></button>)}</div><small>↑↓ جابه‌جایی · Enter انتخاب · Esc بستن</small></div></div>;
}
