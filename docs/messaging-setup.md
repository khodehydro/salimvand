...
placeholderها را دارد: `{customer_name}`, `{invoice_number}`, `{amount}`, `{link}`, `{store}`.
`{customer_name}` به شکل «نام + عزیز» جایگذاری می‌شود (بدون نام: «مشتری گرامی»).
...
در پنل → تنظیمات → «قالب پیامک فاکتور».
متغیرها: `{customer_name}` `{invoice_number}` `{amount}` `{link}` `{store}`.
اگر قالب خالی باشد، متن پیش‌فرض داخلی استفاده می‌شود. برای فاصله‌گذاری بین خطوط،
از اینتر استفاده کنید؛ تایپ کاراکتر `\n` هم به اینتر تبدیل می‌شود.
پیامک فقط هنگام صدور فاکتور و ارسال مجدد (یادآوری بدهی) ارسال می‌شود؛
ثبت پرداخت پیامک ندارد.

## ۴٫۵) انتشار محصول در کانال تلگرام و بله

در پنل → «انبار» → تب «لیست انبار»، هر کارت محصول دکمهٔ «📢 انتشار در شبکه‌ها»
دارد. با کلیک روی آن، پستی شامل عکس محصول، کد، نام، خودروهای سازگار،
شماره تماس و آدرس فروشگاه به کانال‌ها ارسال می‌شود و زیر پست دکمه‌های
شیشه‌ای قرار می‌گیرد: «استعلام و خرید» (پی‌وی ادمین)، «نمایش ویدیو»
(ویدیوی آپارات محصول، در صورت ثبت)، «کاتالوگ محصولات» و «آدرس فروشگاه».

- توکن ربات و آیدی کانال همان مقادیر تب «پیکربندی» است
  (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` و `BALE_BOT_TOKEN`/`BALE_CHAT_ID`).
- ربات باید در کانال **مدیر** باشد، وگرنه تلگرام خطای 403 می‌دهد.
- آیدی پی‌وی ادمین با `SOCIAL_ADMIN_USERNAME` تغییر می‌کند (پیش‌فرض `salimvandiradmin2`).
- تلگرام برای سرور داخل ایران از پروکسی Cloudflare Worker استفاده می‌کند:
  `infra/cloudflare/telegram-proxy` را دیپلوی کنید، سپس در پنل → «پیامک و
  کانال‌ها» → «پیکربندی» دو فیلد «آدرس تلگرام — وورکر کلادفلر» و «رمز
  پروکسی وورکر» را پر کنید (نیازی به تغییر `.env` نیست). بله مستقیم روی
  `tapi.bale.ai` صدا زده می‌شود.
