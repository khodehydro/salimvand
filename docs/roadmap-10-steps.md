...
## گزارش جلسهٔ تکمیل نهایی (۱۴۰۵/۰۶/۰۷ · شاخهٔ arena/01a04dfe-salimvand)

این جلسه با مقایسهٔ کامل اسناد مرجع (سند مادر، سند تحویل بصری و مرجع UI) با کد، حفره‌های باقی‌مانده بسته شد:

| #   | تغییر                      | توضیح                                                                                                                                                                                                                       |
| --- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ۱   | رفع کرش صفحهٔ فاکتور عمومی | `PrintButton` با `onClick` داخل Server Component رندر می‌شد و کل مسیر `/invoice/[token]` و `/i/[code]` خطای ۵۰۰ می‌داد؛ به کامپوننت کلاینت مستقل (`InvoiceActions`) منتقل شد                                                |
| ۲   | صفحهٔ محصول کامل           | گالری با تصاویر بندانگشتی، چیپ وضعیت برای هر ۴ حالت، جدول مشخصات فنی (specs)، شماره فنی، لیست کامل خودروهای سازگار، ویدئوی آپارات با لود پس از کلیک (فیلد `aparatVideoId` به payload عمومی اضافه شد)، JSON-LD و مسیر راهنما |
| ۳   | کارت محصول یکپارچه         | کامپوننت مشترک `ProductCard` در صفحهٔ اصلی، دسته‌بندی و خودرو؛ بج ویدئو و برندها و وضعیت همه‌جا یکسان                                                                                                                       |
| ۴   | بخش تماس صفحهٔ اصلی        | سکشن سرمه‌ای `contact-section` با نقشهٔ زنده، پیام نوسان قیمت و دکمه‌های پیام‌رسان فقط وقتی لینک واقعی تنظیم شده باشد                                                                                                       |
| ۵   | فیلترهای آبشاری خودرو      | برند ← مدل ← تیپ با پارامتر `vehicleMakeId` در URL                                                                                                                                                                          |
| ۶   | فاکتورهای پنل              | اکشن‌های نمایش (درجور عمومی با لینک جدید)، PDF احراز هویت‌شده (`GET /invoices/:id/pdf`)، ستون بدهی، پیل‌های وضعیت پرداخت                                                                                                    |
| ۷   | اتصال صفحات پنل            | پالت Ctrl+K به محصول/مشتری/فاکتور مشخص لینک می‌شود (`#/products?edit=…`، `#/customers?customer=…`، `#/invoices?invoice=…`)؛ دکمهٔ «سایت» در ردیف محصول و «رفتن به انبار» در داشبورد                                         |
| ۸   | تنظیمات پیش‌فرض Seed       | `store.profile`، قالب پیامک، تلگرام/بله، آستانهٔ انبار و بکاپ — پنل و سایت از ابتدای نصب دادهٔ واقعی نشان می‌دهند                                                                                                           |
| ۹   | پیش‌نمایش نقشه در تنظیمات  | پیش‌نمایش زندهٔ همان نقشه‌ای که در سایت دیده می‌شود + خلاصهٔ «چه چیزی کجا نمایش داده می‌شود»                                                                                                                                |
| ۱۰  | CI                         | نسخهٔ اکشن‌های GitHub برای ران‌تایم Node 24 ارتقا یافت (setup-node@v4 روی new runners شکست می‌خورد)                                                                                                                         |

وضعیت راستی‌آزمایی: typecheck هر ۵ پکیج سبز · ۲۱۴ تست سبز (shared ۶ · ui ۷ · api ۱۷۴ · admin ۲۷) · build هر سه اپ موفق · prettier سبز · همهٔ مسیرهای عمومی سایت روی سرور SSR با دادهٔ واقعی بررسی شد.
